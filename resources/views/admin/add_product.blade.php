  ...
  <div class="box span12">
    ...
    <div class="box-content">
      <form class="form-horizontal" action="{{url('/save-product')}}" method="post" enctype="multipart/form-data">
        ...
        <div class="control-group">
        <label class="control-label" for="selectError3">Menufacture Name</label>
        <div class="controls">
          <select id="selectError3" name="menufacture_id">
          <option>--select menufacture--</option>
          <?php
               $all_published_menufacture=DB::table('tbl_menufacture')
                                       ->where('publication_status',1)
                                       ->get();

               foreach($all_published_menufacture as $v_menufacture){?>
            <option value="{{$v_menufacture->menufacture_id}}">{{$v_menufacture->menufacture_name}}</option>
          <?php } ?>
          </select>
        </div>
        </div>
